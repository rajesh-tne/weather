...
def initialize_location_info(location_list):
...
def initialize_weather_data(location_info_list, start_date, api_key):
    weather_data = {}
...
if __name__ == '__main__':
    ...
    location_list = None
    with open('data/sampling_locations.txt') as f:
        location_list = [line.strip() for line in f]

    location_info_list = initialize_location_info(location_list)

    # Generating weeather data
    # Modify year to the year we need for sampling
    weather_data = initialize_weather_data(location_info_list, datetime.datetime(2016, 1, 1), '6dc6eba86807ca3fc046f20df6b9a0ee')

    df = pd.DataFrame(weather_data)
    df.to_csv('data/sampling_weather_data.csv',index=0)
