...
import initialize_training_data
...
def get_forecast_weather_data(location_info_list, temp_model, pres_model, hum_model, cond_model, start_date):
...
if __name__ == '__main__':
    ...
    # sampling data file path here
    df = pd.read_csv('data/sampling_weather_data.csv')
    ...
    # starting date and  generating data every 30 days
    # Modify days as you like
    start_date = datetime.datetime(2017, 1, 1)

    # list of locations for which we are generating data
    location_list = None
    with open('data/forecast_locations.txt') as f:
        location_list = [line.strip() for line in f]

    # obtaining lat, lng and elevation for the listed locations
    location_info_list = initialize_training_data.initialize_location_info(location_list)

    # Generating forecast weather data
    generated_forecast_weather_data = get_forecast_weather_data(location_info_list, temp_model, pres_model, hum_model, cond_model, start_date)

    # Generating dataframe and write to csv
    df = pd.DataFrame(generated_forecast_weather_data)
    df[["Location", "Position", "Local Time", "Conditions", "Temperature", "Pressure", "Humidity"]].to_csv(
        "data/forecast_weather_data.csv",
        header=0,
        index=0,
        sep='|'
    )
